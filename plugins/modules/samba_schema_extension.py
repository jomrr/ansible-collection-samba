...
class SambaSchemaIO:
    ...
    def require_schema_master(self):
        ...
        if ldb.Dn(self.samdb, owner) != ldb.Dn(self.samdb, self.samdb.get_dsServiceName()):
            raise logic.SambaSchemaError(
                f"schema changes must run on the schema master ({owner}); this DC is {self.samdb.get_dsServiceName()}"
            )
    ...
    def find_schema(self, name, attrs):
        ...
        ldb = samba_ldb.load_ldb()
        return self._find(self.schema_dn(), f"(lDAPDisplayName={ldb.binary_encode(name)})", attrs)

    def find_right(self, rights_guid, attrs):
        """The extended right with this rightsGuid, or None."""
        ldb = samba_ldb.load_ldb()
        base = f"CN=Extended-Rights,{self.config_dn()}"
        return self._find(base, f"(rightsGuid={ldb.binary_encode(rights_guid)})", attrs)
    ...
    def apply(self, operations):
        ...
        try:
            ...
            raise logic.SambaSchemaError(f"{label} failed: {samba_ldb.error_text(exc)}") from exc
...
def main():
    """Module entry point."""
    argument_spec = {
        "extension": {"type": "str", "required": True, "choices": sorted(catalog.EXTENSIONS)},
    }
    module = AnsibleModule(argument_spec=argument_spec, supports_check_mode=True)

    fail_without_bindings(module)

    result = run_or_fail(module, "samba_schema_extension", (logic.SambaSchemaError,), lambda: logic.run(module.params, module.check_mode, SambaSchemaIO()))
